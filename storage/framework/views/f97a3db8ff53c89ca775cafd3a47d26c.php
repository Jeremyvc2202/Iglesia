...
<head>
    ...
    <title><?php echo $__env->yieldContent('title', 'Iglesia El Cordero De Dios En El Perú'); ?></title>
    ...
    <header id="site-header" class="sticky top-0 z-50 bg-parchment/90 border-b border-transparent">
        <div class="max-w-4xl mx-auto px-5 sm:px-8">
            <div class="flex items-center justify-between py-4 sm:py-5">
                <a href="<?php echo e(route('anuncios.index')); ?>" class="flex items-center gap-3.5 group focus-visible:rounded-sm">
                    <span class="logo-plate w-14 h-14 rounded-xl flex items-center justify-center p-2 shrink-0">
                        <img src="<?php echo e(asset('img/logo-icon.ico')); ?>" alt="Logo" class="w-full h-full object-contain">
                    </span>
                ...
                <nav class="hidden md:flex items-center gap-6 text-sm font-medium tracking-wide">
                    <a href="<?php echo e(route('anuncios.index')); ?>" class="nav-link animate-color-flow hover:text-wine">Anuncios</a>
    ...
    <aside id="mobile-panel" class="md:hidden fixed top-0 right-0 h-full w-[80%] max-w-xs z-50 flex flex-col">
        ...
        <nav class="flex-1 px-6 py-8 space-y-2">
            <a href="<?php echo e(route('anuncios.index')); ?>" class="mobile-link block py-3 text-base font-medium animate-color-flow hover:text-wine border-b border-hairline/40 transition-colors">Anuncios</a>
    ...
    <?php if (! empty(trim($__env->yieldContent('ocultar_footer')))): ?>
    <?php else: ?>
    <footer class="border-t border-hairline/80 bg-parchment2/10 mt-auto">
        <div class="max-w-4xl mx-auto px-5 sm:px-8 py-8 flex items-center justify-center">
            <span class="text-xs font-medium text-ink/40 tracking-wide text-center">
                &copy; <?php echo e(date('Y')); ?> Iglesia El Cordero de Dios en el Perú. Todos los derechos reservados. | Desarrollado por Jeremy Villacrez.
            </span>
        </div>
    </footer>
    <?php endif; ?>
    ...
    <script>
        ...
        <?php if(session('success') || session('error') || session('warning') || session('info')): ?>
            ...
            <?php if(session('warning')): ?>
                Toast.fire({ icon: 'warning', title: '<?php echo e(session('warning')); ?>' });
            <?php endif; ?>
            <?php if(session('info')): ?>
                Toast.fire({ icon: 'info', title: '<?php echo e(session('info')); ?>' });
            <?php endif; ?>
        <?php endif; ?>
...
</html><?php /**PATH C:\laragon\www\iglesia\resources\views/layouts/app.blade.php ENDPATH**/ ?>